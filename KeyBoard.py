...
# Добавление кнопок для отдельных случаев
books_kb = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)  # Кнопки для книг
for i in books:
    books_kb.add(KeyboardButton(i))

dices = ['d2', 'd3', 'd4', 'd6', 'd8', 'd10', 'd20', 'd100', 'stop']
dices_kb = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)    # Кнопки для костей
for i in dices:
...
classes_kb = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)  # Кнопки для классов
for i in classes:
    classes_kb.add(KeyboardButton(i))

main_func = ['/books', '/roll_dice', '/choose_profile', '/create_profile', '/classes', '/stop', '/spell',
             '/music', '/music_random']
main_func_kb = ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)  # Кнопки для главных команд
for i in main_func:
    main_func_kb.add(i)
...
for i in hero_func:                       # Кнопки для команд взаимодействия с персонажем
    hero_func_kb.add(i)
